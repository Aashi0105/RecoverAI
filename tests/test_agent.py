...
def test_scenario_2_retry_limit_exceeded():
    """SCENARIO 2: Retry limit exceeded -> Policy BLOCKED."""
    txn = build_test_transaction(
    ...
    assert any("limit" in v.lower() or "repeated" in v.lower() for v in res["policy_violations"])
    assert res["action_status"] == "not_executed"


def test_scenario_3_high_value_payment():
    """SCENARIO 3: Transaction > ₹25,000 -> Policy HUMAN_APPROVAL."""
    txn = build_test_transaction(
        transaction_id="txn_test_003",
        customer_id="cust_003",
        merchant_id="merch_01",
        amount=50000.0,  # > ₹25,000 threshold
    ...
    assert res["policy_decision"] == "HUMAN_APPROVAL"
    assert res["agent_status"] == "AWAITING_APPROVAL"
    assert res["action_status"] == "not_executed"


def test_scenario_4_risk_related_failure():
    """SCENARIO 4: Risk-related failure / High IP risk -> Blocked / Escalated."""
    txn = build_test_transaction(
    ...
    assert res["policy_decision"] in ["BLOCKED", "HUMAN_APPROVAL"]
    assert res["action_status"] == "not_executed"


def test_scenario_5_customer_contacted_today():
    """SCENARIO 5: Customer already contacted today -> Policy BLOCKED for reminder/contact."""
    txn = build_test_transaction(
    ...
    assert res["policy_decision"] == "BLOCKED"
    assert any("contact" in v.lower() for v in res["policy_violations"])
    assert res["action_status"] == "not_executed"


def test_scenario_6_low_recovery_probability():
    """SCENARIO 6: Recovery probability < 0.50 -> Policy BLOCKED / no action."""
    txn = build_test_transaction(
...
def test_scenario_7_policy_overrides_llm_recommendation():
    ...
    assert eval_state["policy_decision"] == "BLOCKED"
