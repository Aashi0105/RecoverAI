...
* **Average Treatment Effect (ATE)**: **+1.31 percentage points** (IPW-adjusted).
* **Bootstrap Confidence Intervals**: 1,000 deterministic bootstrap iterations (`seed=42`) yielding **95% CI [-2.50%, +5.56%]** (the interval includes zero, so the effect is not statistically distinguishable from zero).
...
2. ❌ **Observational Causal Analysis**: Propensity Score IPW causal lift analysis (+1.31% IPW-adjusted ATE; 95% Bootstrap CI [-2.50%, +5.56%]) is computed on synthetic historical logs, is not statistically distinguishable from zero in this benchmark, and is NOT a randomized live A/B experiment.
