...
Guarantees that all test and demo transactions contain all 29 APPROVED_MODEL_FEATURES
required by the ML model inference contract, along with agent/policy operational fields.
"""
...
def build_test_transaction(**kwargs) -> Dict[str, Any]:
    """
    Constructs a complete, valid transaction dictionary containing all 29 ML model features
    plus operational fields, applying sensible defaults and internal consistency rules.

    Scenario-specific values can be overridden via kwargs.
    """
    ...
    # Verify all 29 APPROVED_MODEL_FEATURES exist in record
    missing_feats = [f for f in APPROVED_MODEL_FEATURES if f not in record]
    if missing_feats:
        raise ValueError(f"Factory failed to provide required ML features: {missing_feats}")
